...
`AssetOverrides=no` under `[Video]` in `sun.ini` reads every shape from the archives again without moving the files. The setting defaults to `yes`, and a game with no loose shape files draws exactly as before.
